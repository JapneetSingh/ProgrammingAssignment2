...
##This process is especially useful in case where it may be required to compute the inverse repeatedly, like in a loop,
##where the values of the initial matrix don't change much
...
##
##2)cacheSolve():- this function is used to calculate the inverse of the matrix input as an argument into the makeCacheMatrix() and 
##then store its output in the in the same if not already stored . 
...
##If the value is not stored in cache, it will oobtain the matrix using get()function,then use solve() function 
##to calculate the inverse and then store it in the cache matrix using the setinv() 
